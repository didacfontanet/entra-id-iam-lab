...
![Authenticator verification](./05-sspr-verify-step2.png)
...
![SSPR success](./07-sspr-success.png)
...
1. **SSPR has two configuration planes.** Enabling it in Password reset Properties is necessary but not sufficient — the authentication methods must also be enabled in the centralized Authentication methods policy.
2. **Two methods is the right default.** One is insecure, three creates friction that defeats the purpose.
3. **The audit log captures everything**, including failed attempts — valuable for troubleshooting rollouts.
4. **SSPR directly reduces helpdesk load.** Each successful self-service reset is a ticket that never gets created.
5. **Microsoft is centralizing identity controls.** Labs 03 and 04 both documented this transition.
...
- On-premises password writeback requires Entra Connect (not in this lab — cloud-only tenant)
